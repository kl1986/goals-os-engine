...
description: Create or additively refresh <brain>/YYYY-MM-DD.md — today's daily note (Critical, Available time, Now, Call Companion, Drafts to review and send, Later today, Tomorrow candidates, Today's tasks, Daily priorities, Project next actions, Waiting for, Proposed from meetings, Notes). A brand-new day always gets a fresh file, with any still-unchecked lines under Today's tasks, Now, and Later today carried forward into Today's tasks, and unresolved drafts under Drafts to review and send carried into Drafts to review and send (deduplicated on block-id origin marker). Re-running the same day only adds new rows; it never touches or reorders existing lines. Use each morning, or whenever the user wants their daily note (re)generated.
allowed-tools:
  - Bash
triggers:
