---
name: dashboard
description: Regenerate <brain>/Dashboard.md — overdue Routines, pending Triage Plans, open Waiting For items, and today's Action Log summary. Safe to run any time; always fully overwrites the file. Use at the start of a session, or whenever the user wants a status surface.
allowed-tools:
  - Bash
triggers:
  - show my dashboard
...
3. Relay the same summary back in-session (overdue routines, pending plans, open Waiting For items, today's log count) so the user doesn't need to open the file to get the headline — then point them at `Dashboard.md` for the full picture and its links.
