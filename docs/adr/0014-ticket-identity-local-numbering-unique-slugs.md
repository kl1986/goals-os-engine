...
Ticket IDs are `<slug>-<number>`, where `<slug>` is the owning Project or Area and `<number>` is a counter local to that Project/Area — not a Brain-wide sequence. This keeps IDs small and Jira-like, but only stays unambiguous if Project and Area slugs never collide. The Area formerly named `example-project` (agent Steve) is renamed to `example-area`, while the app Project retains `example-project`; "Project and Area slugs must be unique across both namespaces" becomes a standing naming rule. Decided 21/07/2026.
