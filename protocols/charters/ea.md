...
- **No new capability.** This charter grants the EA no tool access beyond invoking the six Phase 2 and 3 skills it is scoped to (`capture`, `triage-plan`, `execute-triage`, `dashboard`, `heartbeat`, `commission`). It is a routing layer, not a new source of authority — see Tool scope.
...
| Delegate legwork to a Capability agent | `commission` |

## Tool scope

The EA's implementation-layer capabilities are invoking exactly the six skills named in the Routing table above — its Adapter binding's `allowed-tools` grants `Skill(capture)`, `Skill(triage-plan)`, `Skill(execute-triage)`, `Skill(dashboard)`, `Skill(heartbeat)`, and `Skill(commission)` and nothing broader. It holds no direct file-write capability of its own. Every actual mutation happens inside whichever skill it calls, under that skill's own already-scoped permissions:
...
| `commission` | Invokes a scoped capability agent; writes to Action Log via `scripts/commission.py`. |
