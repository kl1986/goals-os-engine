...
2. **Rename**, using the file's own location and title:
   - If the file sits at exactly `tasks/projects/<slug>/<file>.md` or `tasks/areas/<slug>/<file>.md`, it's properly filed. The Routine renames it to a slugified version of its own H1 title — no slug or number prefix (ADR-0020). A file with no H1 at all (the `bye.md`/`return-on-constraints-1.md` case — no title, no body) gets a generic `# Untitled ticket` H1 inserted so it isn't left without a title, and is renamed to `untitled-ticket.md`.
   - **Long titles:** a title can be a full free-text sentence; the filename derived from it is truncated to ~60 characters at the last word boundary before the limit. The H1 itself is never truncated, only the filename.
   - **Collisions:** two tickets in the same folder slugifying to the same filename (e.g. two "Untitled ticket" tickets) get a `-2`, `-3`... suffix — exceptional, not the default case.
   - A file with **no inferable location** — e.g. quick-added directly under `tasks/` itself, or under `tasks/projects/` or `tasks/areas/` without a `<slug>/` subfolder at all — is **relocated to `tasks/_unfiled/`**, keeping its own filename (frontmatter backfill from step 1 still applies; only the rename step is skipped). This surfaces it for a human to re-file properly rather than guessing where it belongs.
